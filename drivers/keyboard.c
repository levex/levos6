...
int kbd_read(struct device *dev, void *buf, size_t len, size_t pos)
{
    dev = dev;
    pos = pos;
    ...
    {
        ((char *) buf)[i] = (uint8_t)keyboard_get_key();
    }
    return to_read;
}
...
int keyboard_init()
{
    ...
    kbd_enabled = 1;
    printk("keyboard: setup complete\n");
    return 0;
}
